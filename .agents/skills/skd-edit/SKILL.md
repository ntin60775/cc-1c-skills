...
# /skill:skd-edit — точечное редактирование СКД (Template.xml)
...
- `@off` — установить `<use>false</skill:use>`
- `@on` — убрать существующий `<use>false</skill:use>` (включить параметр)
...
```
/skill:skd-validate <TemplatePath>    — валидация структуры после редактирования
/skill:skd-info <TemplatePath>        — визуальная сводка
```
