...
Скрипты обрабатывают все `.md` файлы в `.agents/skills/*/` (SKILL.md, json-dsl.md и др.). Идемпотентны — повторный запуск безопасен. Python-only навыки (img-grid) пропускаются при переключении на PowerShell.
...
2. Если затронуты inline-утилиты — обновить во всех скриптах: `grep -r "def esc_xml" .agents/skills/`
